...
function loadTask(){
    if(localStorage){
        if(localStorage.myitems){
            itemOperations.itemList= JSON.parse(localStorage.myitems);
...
function saveTask(){
    if(localStorage){
        ...
        localStorage.myitems = jsonString;
    }
}
...
function toggleDelete(){
    var itemObject = itemOperations.searchById(parseInt(this.getAttribute("data-itemno")));
    if(itemObject.isDeleted){
        this.src = "../images/deleted.png";
    }
    else{
      this.src = "../images/del.png";  
    }
    //alert("U Click on Del "+this.getAttribute("data-itemno"));
}
...
function addNewTask(){
    ...
    var img = document.createElement("img");
    img.src="../images/del.png";
    img.setAttribute("data-itemno",itemNo);
    row.insertCell(4).appendChild(img);
    img.addEventListener("click",toggleDelete);
    //row.insertCell(4).innerHTML = "<img src ='../images/del.png'>"
    itemNo++;
    clearField();
}
